...
describe("course player navigation", () => {
  it("preserves Courses as the launch origin", () => {
    const origin = getCoursePlayerOriginFromPathname("/courses");
    ...
    expect(getCoursePlayerOrigin("?from=courses")).toBe("courses");
    expect(getCoursePlayerSection(origin)).toBe("Courses");
    expect(getCoursePlayerParentPath(origin)).toBe("/courses");
    expect(getCoursePlayerBackLabel(origin)).toBe("Return to Courses");
    expect(getCoursePlayerOriginFromPathname("/courses/")).toBe("courses");
  });
  ...
  it("maps every course-launching surface to its own player origin", () => {
    ...
    expect(getCoursePlayerOrigin("")).toBe("courses");
    ...
    expect(getCoursePlayerParentPath("courses")).toBe("/courses");
    expect(getCoursePlayerParentPath("wishlist")).toBe("/wishlist");
    expect(getCoursePlayerSection("home")).toBe("Home");
    expect(getCoursePlayerSection("courses")).toBe("Courses");
    ...
    expect(getCoursePlayerBackLabel("courses")).toBe("Return to Courses");
  ...
  it("keeps only the latest learning session and moves its source indicator", () => {
    expect(
      rememberCoursePlayerDestination("course one", "courses"),
    ).toBe(
      "/learn/course%20one/the-beginning-of-a-design-journey?from=courses",
    );
    expect(getResumableCoursePlayerNavigationPath("/courses")).toBe(
      "/learn/course%20one/the-beginning-of-a-design-journey?from=courses",
    );

    expect(rememberCoursePlayerDestination("course-two", "courses")).toBe(
      "/learn/course-two/the-beginning-of-a-design-journey?from=courses",
    );
    expect(getResumableCoursePlayerNavigationPath("/courses/")).toBe(
      "/learn/course-two/the-beginning-of-a-design-journey?from=courses",
    );
    ...
    expect(getActiveCoursePlayerSession()).toMatchObject({
      ...
      lessonId: 1,
      origin: "courses",
      path: "/learn/course-two/the-beginning-of-a-design-journey?from=courses",
    });

    expect(getRememberedCoursePlayerDestination("courses")).toBe(
      "/learn/course-two/the-beginning-of-a-design-journey?from=courses",
    );
    clearRememberedCoursePlayerDestination("courses");

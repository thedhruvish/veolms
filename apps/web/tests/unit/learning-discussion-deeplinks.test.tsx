...
describe("Learning Space Discussion Deep-Linking", () => {
        ...
        threads: [
          {
            ...
            lessonId: "lesson-1",
            userId: "user-1",
            author: { id: "user-1", displayName: "Feed Author 1", username: "author1" },
            kind: "comment",
          ...
          },
          {
            ...
            lessonId: "lesson-1",
            userId: "user-2",
            author: { id: "user-2", displayName: "Feed Author 2", username: "author2" },
            kind: "question",
  ...
  describe("1. Course Player Navigation thread utilities", () => {
    it("extracts thread parameter correctly from search string", () => {
      ...
      expect(getCoursePlayerThread("?from=courses&returnTo=%2Fcourses&thread=thr-abc")).toBe("thr-abc");
  ...
  describe("2. Mock Mode direct-thread deep link and error handling", () => {
    it("opens discussion thread panel when URL has matching mock thread ID (?thread=4)", async () => {
      render(
        <MemoryRouter initialEntries={["/learn/test-course/lecture-1?thread=4"]}>
      ...
      expect(screen.getAllByText("Rohit Sharma").length).toBeGreaterThanOrEqual(1);
    });

    it("shows error notice and removes ?thread= with replace when thread is not found in mock mode", async () => {
      let currentLoc: any;
      render(
        <MemoryRouter initialEntries={["/learn/test-course/lecture-1?from=courses&thread=non-existent-99999"]}>
      ...
      await waitFor(() => {
        expect(
          screen.getByText("This discussion thread is unavailable or has been removed."),
        ).toBeInTheDocument();
      });

      // Panel should NOT be open
      expect(screen.queryByRole("region", { name: /swipe between discussion threads/i })).not.toBeInTheDocument();
  ...
  describe("3. Backend Mode direct-thread fetch states (loading, success, error)", () => {
    it("loading state: does NOT open feed entry 0 or show error notice while direct thread fetch is pending", async () => {
      ...
      render(
        <MemoryRouter initialEntries={["/learn/course-1/lecture-1?thread=thr-in-flight"]}>
          <Discussion persistenceKey="test-backend" courseId="course-1" lessonId="1" />
        </MemoryRouter>,
      );

      // Thread details query was triggered with thread ID
      expect(mockInteractions.useThreadDetails).toHaveBeenCalledWith("thr-in-flight", { enabled: true });
      ...
      expect(
        screen.queryByText("This discussion thread is unavailable or has been removed."),
    ...
    it("success state: opens exact fetched direct thread even if it was not in the paginated lesson feed", async () => {
      const directThread: LearningThread = {
        ...
        lessonId: "lesson-1",
        userId: "user-ext",
        author: { id: "user-ext", displayName: "External Author", username: "extauthor", role: "Student" },
        kind: "comment",
      ...
      render(
        <MemoryRouter initialEntries={["/learn/course-1/lecture-1?thread=thr-external-99"]}>
          <Discussion persistenceKey="test-backend" courseId="course-1" lessonId="1" />
        </MemoryRouter>,
      );
      ...
      expect(screen.getByText("External direct link comment body")).toBeInTheDocument();
    });
    ...
    it("error state: shows notice and clears only thread param with replace=true when direct thread fails to load", async () => {
      ...
      render(
        <MemoryRouter initialEntries={["/learn/course-1/lecture-1?from=courses&returnTo=%2Fcourses&thread=thr-404"]}>
          <LocationProbe onLocation={(loc) => (currentLoc = loc)} />
          <Discussion persistenceKey="test-backend" courseId="course-1" lessonId="1" />
        </MemoryRouter>,
      );
  ...
  describe("4. In-page URL sync (Opening, Closing, and Browser Navigation)", () => {
    it("updates URL with ?thread=<id> when clicking a discussion entry in feed", async () => {
      let currentLoc: any;
      render(
        <MemoryRouter initialEntries={["/learn/course-1/lecture-1?from=courses"]}>
          <LocationProbe onLocation={(loc) => (currentLoc = loc)} />
          <Discussion persistenceKey="test-backend" courseId="course-1" lessonId="1" />
        </MemoryRouter>,
      );
    ...
    it("removes ?thread= when thread panel close button is clicked", async () => {
      let currentLoc: any;
      render(
        <MemoryRouter initialEntries={["/learn/course-1/lecture-1?thread=thr-feed-1"]}>
          <LocationProbe onLocation={(loc) => (currentLoc = loc)} />
          <Discussion persistenceKey="test-backend" courseId="course-1" lessonId="1" />
        </MemoryRouter>,
      );
      ...
      const closeButton = screen.getByRole("button", { name: /close discussion thread/i });
  ...
  describe("5. Share action link generation", () => {
    it("copies canonical URL with ?thread=<rootId> and no hash for root threads", async () => {
      ...
      window.location = new URL("https://procodrr.local/learn/my-course/lecture-1?from=courses") as any;
      ...
      await shareDiscussionEntry("thr-root-123", "Alice", "Root thread body text");
    ...
    it("copies URL with ?thread=<parentId>#discussion-entry-<replyId> for replies", async () => {
      ...
      expect(writeTextMock).toHaveBeenCalledTimes(1);
      const copied = writeTextMock.mock.calls[0]![0];
      expect(copied).toContain("thread=thr-root-123#discussion-entry-reply-456");

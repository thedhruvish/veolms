...
`);
  ...
  const addedDockerEnvKeys: string[] = [];
  if (selectedProvider.id === "docker") {
    ...
    // Compose reads this same file. Copy the actual configured database URL so
    // no Compose-managed PostgreSQL hostname is inferred or hard-coded.
    if (!getEnvValue(envContent, "FLEET_DATABASE_URL")) {
      envContent = setEnvValue(envContent, "FLEET_DATABASE_URL", databaseUrl);
      addedDockerEnvKeys.push("FLEET_DATABASE_URL");
    }
    if (!getEnvValue(envContent, "DOCKER_WORKER_DATABASE_URL")) {
      envContent = setEnvValue(
        envContent,
        "DOCKER_WORKER_DATABASE_URL",
        databaseUrl,
      );
      addedDockerEnvKeys.push("DOCKER_WORKER_DATABASE_URL");
    }
  ...
  if (selectedProvider.id === "docker") {
    console.info(
      addedDockerEnvKeys.length > 0
        ? `  Added missing Docker Fleet env keys: ${addedDockerEnvKeys.join(", ")}. Database values come from DATABASE_URL.`
        : "  Docker Fleet environment is already configured; no existing values were changed.",
    );

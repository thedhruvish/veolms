...
| **`@veolms/database`**             | [`packages/database`](../packages/database)                         | Kysely database client, migration [`003-create-fleet-manager-tables.ts`](../packages/database/migrations/003-create-fleet-manager-tables.ts) with `jobs`, `workers`, `worker_monitoring`, and `worker_events`. |

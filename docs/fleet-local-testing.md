...
set `FLEET_DATABASE_URL` to the database the manager and workers should use.
...
# fills missing Fleet Docker keys from DATABASE_URL in
# apps/fleet-manager/.env. Edit those keys there only when necessary.
...
```bash
# `pnpm fleet:provider` has already added the required Fleet database keys to
